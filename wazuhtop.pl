...
# Version 0.1 (09/2006)
#
# Migration to WAZUH EDR: Adam 'tick' Pielak (tick@linuxmafia.pl)
# File: wazuhtop.pl
# Version 0.2 (01/2023)
#
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
#
# WAZUH XDR - The Open Source Security Platform.
# Based on OSSEC HIDS Copyright (C) 2014 Trend Micro Inc.
...
my $sig_class_id=1;
my $dump=0;
my ($hids_id,$hids,$hids_interface,$last_cid)=(undef, 'localhost', 'wazuh',0);
...
my $delta=0;
########################################################
my $logfile="alerts.log";
my $LOG='/var/ossec/logs/alerts/'. $logfile;
chomp $LOG;
$date='';
format TOPREPORT =
.==========================================================================================================================.
...
sub taillog {
   ...
   while (1==1) {
	...
	%resolv=();
       ...
       $stall += 1;
        $logfile="alerts.log";
	$LOG='/var/ossec/logs/alerts/'. $logfile;
	chomp $LOG;
...
sub version(){
	print "[ WazuhTOP - (C) 2006-2023 $VERSION ]\n";
	print "  Released under the GNU GPL.\n\n";
}

sub help(){
	&version();
	print "\tList alerts generated by Wazuh.\n\n";
        print "USAGE:\n";
        print "$0 -h, --help     display this help and exit\n\n";
	print "OPTIONS:\n";
	print "$0 -n, --noname   not try to resolve the host names\n\n";
	print "Long options may be passed with a single dash.\n\n";
        print "Visit https://www.wazuh.com\n";
	exit 0;
}
